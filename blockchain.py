...
class PropertyBlockchain:
    ...
    def register_or_validate_identity(self, owner: str, aadhar: str, pan: str) -> bool:
        ...
        self._log(f"Identity registered: {owner_normalized} | Customer Key: {customer_key}")
        return True
    ...
    def add_property(self, property_key: str, owner: str, address: str, 
                     pincode: str, value: float, aadhar_no: str, pan_no: str,
                     survey_no: str, rtc_no: str = "",
    ...
    def transfer_property(self, property_key: str, new_owner: str,
                          new_owner_aadhar: str, new_owner_pan: str,
        ...
        new_owner_customer_key = self.identity_registry[new_owner_normalized]["customer_key"]
        ...
        self._log(f"Property '{property_key}' transferred from '{previous_owner}' to '{new_owner}' ({transfer_reason})")
        return new_block
    
    def inherit_property(self, property_key: str, deceased_owner: str, heir: str,
                         heir_aadhar: str, heir_pan: str,
                         relationship: str = "", 
    ...
    def search_by_owner_details(self, customer_key: str, pan: str, aadhar: str) -> List[Dict[str, Any]]:
        """
        ...
        for property_key in self.property_index:
            try:
                state = self.get_property_current_state(property_key)
                if (
                    state["customer_key"] == customer_key and
                    state["pan_no"] == pan.upper() and
                    state["aadhar_no"] == aadhar.replace(" ", "").replace("-", "")
                ):
                    results.append(state)
            except:
                continue
    ...
    def _calculate_fuzzy_score(self, query: str, target: str) -> float:
        ...
        matching_words = sum(1 for qw in query_words if any(qw in tw or tw in qw for tw in target_words))
        if query_words:
    ...
    def unified_search(self, query: str) -> List[Dict[str, Any]]:
        ...
        for property_key in self.property_index:
            ...
            try:
                ...
                searchable_fields = {
                    "property_key": {"value": state.get("property_key", ""), "weight": 1.0, "normalize": False},
                    "owner": {"value": state.get("owner", ""), "weight": 1.0, "normalize": False, "fuzzy": True},
                    "customer_key": {"value": state.get("customer_key", ""), "weight": 0.95, "normalize": False},
                    "survey_no": {"value": state.get("survey_no", ""), "weight": 0.9, "normalize": False},
                    "rtc_no": {"value": state.get("rtc_no", ""), "weight": 0.9, "normalize": False},
                    "aadhar_no": {"value": state.get("aadhar_no", ""), "weight": 0.85, "normalize": True},
                    "pan_no": {"value": state.get("pan_no", ""), "weight": 0.85, "normalize": False},
                    "village": {"value": state.get("village", ""), "weight": 0.7, "normalize": False},
                    "district": {"value": state.get("district", ""), "weight": 0.7, "normalize": False},
                    "taluk": {"value": state.get("taluk", ""), "weight": 0.7, "normalize": False},
                    "pincode": {"value": state.get("pincode", ""), "weight": 0.8, "normalize": False},
                    "address": {"value": state.get("address", ""), "weight": 0.6, "normalize": False},
                }
                ...
                for field_name, field_config in searchable_fields.items():
                    ...
                    if field_config.get("normalize"):
                        field_value_clean = field_value.replace(" ", "").replace("-", "").lower()
                    ...
                    # Starts with query
                        ...
                        score = 90.0 + (len(compare_query) / len(field_value_clean)) * 10
                    # Contains query
                    elif compare_query in field_value_clean:
                        score = 75.0 + (len(compare_query) / len(field_value_clean)) * 15
    ...
    def get_all_properties(self) -> List[Dict[str, Any]]:
        ...
        return [self.get_property_current_state(key) 
                for key in self.property_index if key != "GENESIS"]
    ...
    def _save_blockchain(self) -> bool:
        ...
        try:
            ...
            if os.path.exists(self.STORAGE_FILE):
                try:
                    subprocess.run(['attrib', '-R', self.STORAGE_FILE], 
            ...
            self._log(f"Blockchain saved to read-only encrypted file: {self.STORAGE_FILE}")
            return True
    ...
    def _load_blockchain(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                self._log(f"Parsed JSON successfully, found {len(blockchain_data.get('chain', []))} blocks")
            except json.JSONDecodeError as json_err:
                self._log(f"JSON parse failed: {json_err}", "error")
                self._log(f"First 100 chars of decrypted data: {json_data[:100]}", "error")
                return False
    ...
    def backup_to_ipfs(self) -> Optional[str]:
        ...
        try:
            ...
            if not self.PINATA_API_KEY or not self.PINATA_SECRET_KEY or \
               self.PINATA_API_KEY == "your_pinata_api_key_here" or \
               self.PINATA_SECRET_KEY == "your_pinata_secret_key_here":
                self._log("ERROR: Pinata API keys not configured! Please set PINATA_API_KEY and PINATA_SECRET_KEY environment variables.", "error")
                return None
            ...
            with open(self.STORAGE_FILE, 'rb') as file:
                ...
                response = requests.post(url, files=files, headers=headers, data=payload, timeout=60)
        ...
        try:
            ...
            for url in urls_to_try:
                try:
                    ...
                    self._log(f"Status: {response.status_code}, Size: {len(response.content)} bytes")
                    
                    if response.status_code == 200 and len(response.content) > 1000:
                        temp_content = response.content
                        # Check if valid base64 encrypted data
                        try:
                            text_start = temp_content[:100].decode('utf-8', errors='strict')
                            if re.match(r'^[A-Za-z0-9+/=]+', text_start):
                                self._log(f"✓ Valid encrypted data received! ({len(temp_content)} bytes)")
                                content = temp_content
                                break
                            elif '<html' in text_start.lower() or '<!doctype' in text_start.lower():
                        ...
                        self._log(f"File too small ({len(response.content)} bytes), trying next...")
            ...
            if content is None:
                self._log(f"All download methods failed. Last error: {last_error}", "error")
                self._log("The file might be unpinned or the CID format is incorrect.", "error")
                return False
            
            self._log(f"Downloaded {len(content)} bytes")
            
            if len(content) < 1000:
                self._log(f"Downloaded file too small ({len(content)} bytes), not a valid blockchain backup", "error")
                return False
